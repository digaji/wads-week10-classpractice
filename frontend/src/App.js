...
class App extends React.Component {
  ...
  async getAllHero() {
    ...
    this.setState(state => ({items: data.map(dat => ({_id: dat._id, heroname: dat.heroname}))}));
  }
  ...
  render() {
    ...
    return (
      <div className="App">
        <h1>Hero Keeper</h1>
        <p>List all your favorite heros here</p>
        <ul>{items}</ul>
        <input
          onChange={(e) => {
          ...
          onKeyPress={async (e) => {
            if ((e.key === "Enter") && (this.state.inputField !== "")) {
              ...
              this.setState(state => ({items: state.items.concat({_id: res.data, heroname: heroname})}));
            }
          }}
